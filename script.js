...
async function initializeAudio() {
    ...
    if (!hitSynth) {
        hitSynth = new Tone.Synth({ oscillator: { type: "square" }, envelope: { attack: 0.001, decay: 0.1, sustain: 0, release: 0.1 } }).toDestination();
        missSynth = new Tone.NoiseSynth({ envelope: { attack: 0.005, decay: 0.15, sustain: 0 } }).toDestination();
...
// --- GAME LOGIC ---
let hits=0, misses=0, shotsImg=0, missesImg=0, spawnInterval, gameTime=30, timerInterval, isGameRunning=false;
...
window.addEventListener('resize', ()=>{
    if(isGameRunning){document.querySelectorAll('.target').forEach(t=>t.remove()); spawnTarget();}
});
...
uploadInput.addEventListener('change',(e)=>{
    ...
    if(file){const reader=new FileReader(); reader.onload=(ev)=>{currentTargetImg=ev.target.result;}; reader.readAsDataURL(file); document.querySelectorAll('.target').forEach(t=>t.src=currentTargetImg);}
});
...
gameArea.addEventListener('mousemove',(e)=>{
    ...
    sandalCursor.style.left=`${e.clientX}px`; sandalCursor.style.top=`${e.clientY}px`;
    fireCursor.style.left=`${e.clientX}px`; fireCursor.style.top=`${e.clientY}px`;
});
...
gameArea.addEventListener('click',(e)=>{
    ...
    sandalCursor.classList.add('shoot'); fireCursor.classList.add('shoot');
    ...
    if(target){
        ...
        if(e.clientX>=rect.left && e.clientX<=rect.right && e.clientY>=rect.top && e.clientY<=rect.bottom){
            hits++; shotsImg++; playHit(); showHitFlash(e.clientX,e.clientY); showShot(target);
        } else {misses++; missesImg++; playMiss(); showMiss(e.clientX,e.clientY); shakeGameArea();}
    } else {misses++; missesImg++; playMiss(); showMiss(e.clientX,e.clientY); shakeGameArea();}
    updateScore();
});

function shakeGameArea(){gameArea.classList.add('shake'); setTimeout(()=>gameArea.classList.remove('shake'),200);}
function spawnTarget(){
    document.querySelectorAll('.target').forEach(t=>t.remove());
    const target=document.createElement('img');
    target.src=currentTargetImg; target.classList.add('target');
    ...
    target.style.width=`${finalSize}px`; target.style.height=`${finalSize}px`;
...
function showShot(target){
    ...
    shot.style.position='absolute'; shot.style.left=target.style.left; shot.style.top=target.style.top;
    shot.style.width=target.style.width; shot.style.height=target.style.height; shot.style.pointerEvents='none';
    gameArea.appendChild(shot); target.remove();
    setTimeout(()=>shot.remove(),300);
}
function showMiss(x,y){
    const miss=document.createElement('img'); miss.src=missImgSrc; miss.classList.add('miss-fx');
    const rect=gameArea.getBoundingClientRect();
    miss.style.left=`${x-rect.left-20}px`; miss.style.top=`${y-rect.top-20}px`;
    gameArea.appendChild(miss); setTimeout(()=>miss.remove(),300);
}
function showHitFlash(x,y){
    const flash=document.createElement('div'); flash.classList.add('hit-fx');
    const rect=gameArea.getBoundingClientRect(); flash.style.left=`${x-rect.left}px`; flash.style.top=`${y-rect.top}px`;
...
function updateScore(){
    ...
    imageCountDisplay.textContent=`Target Hits: ${shotsImg} | Game Misses: ${missesImg}`;
}
function startGame(){
    if(isGameRunning) return;
    isGameRunning=true; gameOverModal.classList.add('hidden');
